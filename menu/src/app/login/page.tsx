...
export default function LoginPage() {
  ...
  async function handleGoogle() {
    const err = await loginWithSupabase();
    if (err) setError(err);
  }
    ...
    <div className="flex min-h-screen items-center justify-center bg-canvas px-4 py-10">
      <div className="w-full max-w-sm">
        ...
        <div className="mb-8 text-center">
          ...
          <h1 className="text-3xl font-medium tracking-tight text-ink">Menu.AI</h1>
        ...
        <div className="rounded-xl border border-hairline bg-white p-6 shadow-sm">
          <button
            ...
            className="flex min-h-11 w-full items-center justify-center gap-2 rounded-lg border border-hairline bg-white px-4 py-2.5 text-sm font-medium text-ink shadow-sm transition-colors hover:bg-surface-soft focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-info-border"
          >
          ...
          <div className="relative my-5">
            ...
            <div className="relative flex justify-center text-xs"><span className="bg-white px-2 text-ink-muted-48">ou</span></div>
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label htmlFor="email" className="mb-1.5 block text-xs font-medium text-ink-muted-80">Email</label>
              <input
                id="email"
                type="email"
                value={email}
            ...
            <div>
              <label htmlFor="senha" className="mb-1.5 block text-xs font-medium text-ink-muted-80">Senha</label>
        ...
        <p className="mt-6 text-center text-xs text-ink-muted-48">
          Não tem conta?{" "}
          <a href="/register" className="font-medium text-link hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-info-border focus-visible:ring-offset-2">Criar conta</a>
        </p>
      </div>
    </div>
  );
